...
namespace PXS.IfRF.Supporting
{
	...
	public static class ExtensionMethods
	{
		...
		public static void Dump( this Microsoft.AspNetCore.Http.IHeaderDictionary headers, ILoggerManager logger)
		{

			logger.Debug( $"SDE:Dumping headers");
			logger.Debug( $"SDE:{headers.Count} headers");

			int i = 1;
			foreach(var item in headers)
			{
				logger.Debug( $"SDE:{i++}:{item.Key} ={item.Value}");
			}
		}

...
where subrack.id = :sr_id";
		...
		internal static string getCustomerReference( Connection c)
		{
			...
			if( c.From != null)
			{
				if( c.Type == "FD-C")
				{
					if( c.From.Type == "PP")
					{
						...
						customerReference = $"{c.FromPopName}-{c?.From?.Subrack?.RackSpace?.FrameId}-{c.FromSrName}-{c?.From?.PositionId}";
					}
				}
			}
			...
			if( c.To != null)
			{
				if( c.Type == "FD-C")
				{
					if( c.To.Type == "PP")
					{
						...
						customerReference = $"{c.ToPopName}-{c?.To?.Subrack?.RackSpace?.FrameId}-{c.ToSrName}-{c?.To?.PositionId}";
					}
				}
			}

...
namespace PXS.IfRF.AuthHandling
{
        ...
        : AuthenticationHandler<WebsealAuthenticationOptions>
    {
        ...
        )
        {
            ...
            _logger.Debug( "Entering [GetCertificateAuthorization]");
            ...
            string connectionId = Request.HttpContext.Connection.Id;
            string remoteHost   = Request.HttpContext.Request.Headers[ "x-forwarded-host"].FirstOrDefault();
            string remoteIp     = Request.HttpContext.Connection.RemoteIpAddress.ToString();            

            _logger.Debug( $"processing connectionId=[{connectionId}], remoteHost=[{remoteHost}], remoteIp=[{remoteIp}], ");
            
            //Request.HttpContext.Request.Headers.Dump( _logger);

            if( clientCert == null)
            {
                _logger.Debug( $"[{connectionId}/{remoteIp}]:Client is not using a certificate, defaulting to WebSeal");
                return( CertificateAuthorizationType.Webseal);
                //return( CertificateAuthorizationType.Invalid);
            }

            _logger.Debug( $"[{connectionId}/{remoteIp}]:Client is using a certificate");
            ...
            _logger.Debug( $"[{connectionId}/{remoteIp}]:subjectCN={subjectCN}");

            _logger.Debug( $"[{connectionId}/{remoteIp}]:dumping allowedCN, {allowedCN.Count} items");

            int i = 1;
            foreach( var item in allowedCN)
            {
                _logger.Debug( $"allowedCN {i++}={item}");
            }

            if (!(allowedCN.Any(cn => cn.ToUpper().Contains( theSubject))))
            {
                _logger.Debug( $"[{connectionId}/{remoteIp}]:subjectCN={subjectCN} not in 'allowedCN'");
                return CertificateAuthorizationType.Invalid;
            }
            ...
            if (subjectCN.StartsWith("INTRA.") && subjectCN.EndsWith(".WEB.BC"))
            {
                ...
                _logger.Debug( $"[{connectionId}/{remoteIp}]:subjectCN={subjectCN} 'INTRA'/'.WEB.BC'");
                return CertificateAuthorizationType.Webseal;
            }
            ...
            {
                // webseal, will use http headers to get roles
                 _logger.Debug( $"[{connectionId}/{remoteIp}]:subjectCN={subjectCN} 'IFRF-DEV'");
                return CertificateAuthorizationType.Webseal;
            }

            // no webseal, will use first part of CN to get roles
            groupFromCN = subjectCN.Split("-")[0];
             _logger.Debug( $"[{connectionId}/{remoteIp}]:groupFromCN={groupFromCN}");
